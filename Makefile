...
help:
	@echo "targets:"
	@echo "  clean        仅删本仓生成物(bin/.run/output/coverage + swaggo docs)"
...
# 生成物目录（clean 与 clean-deep 都会删；绝不删源码 / 下载依赖）
...
clean:
	...
	# 散落单文件生成物
	find $(CURDIR) -maxdepth 2 \( -name '*.test' -o -name '*.out' -o -name '.DS_Store' -o -name '*.swp' \) -delete 2>/dev/null
	rm -f $(CURDIR)/docs/docs.go $(CURDIR)/docs/swagger.json $(CURDIR)/docs/swagger.yaml
	@echo "[clean:$(APP)] done."
